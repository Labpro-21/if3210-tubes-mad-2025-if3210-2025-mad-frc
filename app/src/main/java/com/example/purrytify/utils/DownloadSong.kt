...
fun downloadSong(
    ...
    CoroutineScope(Dispatchers.IO).launch {
        ...
        if (already) {
            Log.i(
                tag,
                "Skip download – sudah ada local song with serverId=${songToDownload.serverId}"
            )
        ...
        withContext(Dispatchers.Main)
        {
            try {
                ...
                val safeTitleAudio =
                    songToDownload.title.replace(Regex("[^a-zA-Z0-9\\s]"), "_").take(50)
                val safeArtistAudio =
                    songToDownload.artist.replace(Regex("[^a-zA-Z0-9\\s]"), "_").take(50)
                ...
                val receiver = object : BroadcastReceiver() {
                    ...
                    override fun onReceive(ctx: Context, intent: Intent) {
                        ...
                        Log.d(
                            tag,
                            "BroadcastReceiver - Received download ID: $receivedId, Expected ID: $downloadId"
                        )
                        ...
                        if (receivedId == downloadId) {
                                        ...
                                        if (statusIndex != -1 && localUriIndex != -1) {
                                            ...
                                            when (status) {
                                                DownloadManager.STATUS_SUCCESSFUL -> {
                                                    Log.i(
                                                        tag,
                                                        "Song download SUCCESSFUL. Local URI: $localSongUriString"
                                                    )
                                                    successful = true
                                                }
                                                ...
                                                DownloadManager.STATUS_FAILED -> {
                                                    Log.e(
                                                        tag,
                                                        "Song download FAILED. Reason: $reason, Local URI: $localSongUriString"
                                                    )
                                                }

                                                DownloadManager.STATUS_PAUSED -> Log.w(
                                                    tag,
                                                    "Song download PAUSED. Reason: $reason"
                                                )

                                                DownloadManager.STATUS_PENDING -> Log.d(
                                                    tag,
                                                    "Song download PENDING."
                                                )

                                                DownloadManager.STATUS_RUNNING -> Log.d(
                                                    tag,
                                                    "Song download RUNNING."
                                                )

                                                else -> Log.w(
                                                    tag,
                                                    "Unknown song download status: $status"
                                                )
                                            }
                                        } else {
                                            Log.e(
                                                tag,
                                                "ERROR: Critical column indices not found in cursor."
                                            )
                                        }
                                    } else {
                                        Log.e(
                                            tag,
                                            "ERROR: Cursor is null or empty for song download query."
                                        )
                                    }
                                }
                            } catch (e: Exception) {
                            ...
                            if (successful && localSongUriString != null) {
                                CoroutineScope(Dispatchers.IO).launch {
                                    var finalArtworkPath: String? = songToDownload.artworkPath
                                    if (!songToDownload.artworkPath.isNullOrEmpty() && songToDownload.artworkPath.startsWith(
                                            "http"
                                        )
                                    ) {
                                        Log.d(
                                            tag,
                                            "Attempting to download artwork from: ${songToDownload.artworkPath}"
                                        )
                                        try {
                                            val safeTitleArtwork = songToDownload.title.replace(
                                                Regex("[^a-zA-Z0-9\\s]"),
                                                "_"
                                            ).take(50)
                                            ...
                                            URL(songToDownload.artworkPath).openStream()
                                                .use { input ->
                                            ...
                                            Log.i(
                                                tag,
                                                "Artwork downloaded successfully to: $finalArtworkPath"
                                            )
                                        } catch (e: Exception) {
                                    ...
                                    Log.d(
                                        tag,
                                        "Preparing to add new song to database: $newDownloadedSong"
                                    )
                                    songViewModel.addSong(newDownloadedSong)
                                    Log.i(
                                        tag,
                                        "New song '${newDownloadedSong.title}' added to database."
                                    )
                                    onComplete()
                                }
                            } else {
                                ...
                                Log.w(
                                    tag,
                                    "Song download was not successful or local URI is null. Calling onComplete."
                                )
                                onComplete()
                            }
                            ...
                            try {
                                ...
                                Log.e(
                                    tag,
                                    "ERROR unregistering receiver (already unregistered?): ${e.message}",
                                    e
                                )
                            } catch (e: Exception) {
